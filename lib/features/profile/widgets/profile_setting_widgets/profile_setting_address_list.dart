...
class ProfileSettingAddressList extends GetWidget<GetUserAddressController> {
      ...
      return controller.isLoading.value
          ...
          : Column(
              children: List.generate(
                controller.userAddress.value?.data?.length ?? 0,
                (index) {
                  final item = controller.userAddress.value?.data?[index];
                  return SharedContainer(
                    padding: EdgeInsets.all(16.r),
                    radius: 16.r,
                    margin: EdgeInsets.only(
                      bottom:
                          controller.userAddress.value!.data!.length - 1 ==
                              index
                          ? 0
                          : 12.h,
                    ),
                    ...
                    child: Row(
                      ...
                      children: [
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        border: Border.all(
                                          ...
                                          color: isDark
                                              ? AppColors.darkBorderColor
                                              : AppColors.darkColor.withValues(
                                                  alpha: 0.08,
                                                ),
                                        ),
                                      ),
                        ...
                        if (item?.isDefault != true)
                          TextButton(
                            onPressed: () async {
                              await Get.find<UpdateAddressController>()
                                  .updateAddress(
                                    addressID: item?.id ?? 0,
                                    isDefault: true,
                                  );
                        ...
                        InkWell(
                          onTap: () async {
                            await Get.find<DeleteAddressController>()
                                .deleteAddress(addressID: item?.id ?? 0);
                          },
                          child: SizedBox(
                            height: 36.h,
                            width: 40.w,
